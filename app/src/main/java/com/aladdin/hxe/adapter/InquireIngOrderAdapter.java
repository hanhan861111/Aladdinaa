...
import com.aladdin.hxe.holder.InquireIngOrderHolder;
...
public class InquireIngOrderAdapter extends RecyclerView.Adapter<InquireIngOrderHolder> {
    ...
    public InquireIngOrderAdapter(Context context) {
    ...
    @Override
    public InquireIngOrderHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View inflate = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_inquire_allorder, parent, false);
        return new InquireIngOrderHolder(inflate);
    }

    @Override
    public void onBindViewHolder(final InquireIngOrderHolder holder, final int position) {
        if (rowsBean.size() > 0) {
            //判断是否是拼团

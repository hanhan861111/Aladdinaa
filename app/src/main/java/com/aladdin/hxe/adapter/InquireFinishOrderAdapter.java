...
public class InquireFinishOrderAdapter extends RecyclerView.Adapter<InquireFinishOrderHolder> {
    ...
    public void setDatas(ArrayList<InquireFinishOrderBean.DataBean.RowsBean> rowsBean) {
        this.rowsBean = rowsBean;
        notifyDataSetChanged();
    }

    @Override
    public InquireFinishOrderHolder onCreateViewHolder(ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(final InquireFinishOrderHolder holder, final int position) {
        if (rowsBean.size() > 0) {
            //判断是否是拼团
